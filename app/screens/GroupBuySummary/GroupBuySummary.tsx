...
const GroupBuySummary = ({ navigation, route }: Props) => {
                ...
                try {
                    ...
                    if (data) {
                        ...
                        const contributionsWithUsers = await Promise.all(
                            data.contributions.map(async (contribution) => {
    ...
    const handleToggleAcceptingMembers = async () => {
        ...
        try {
            ...
            setGroupBuy(prev => prev ? { ...prev, acceptingNewMembers: !prev.acceptingNewMembers } : null);
            Alert.alert('Success', `GroupBuy is now ${groupBuy.acceptingNewMembers ? 'closed' : 'open'} to new members.`);
        } catch (error: any) {
                ...
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            setGroupBuy(prev => prev ? { ...prev, status: 'finished', acceptingNewMembers: false } : null);
    ...
    const getStatusText = () => {
        ...
        if (!groupBuy.acceptingNewMembers) return 'Closed to New Members';
        return 'Active';
    };
                        ...
                        <View className="border-t border-gray-200 pt-4">
                            ...
                            <View className="flex-row flex-wrap gap-2">
                                <TouchableOpacity
                                    ...
                                    style={{ backgroundColor: groupBuy.acceptingNewMembers ? '#ef4444' : Colors.primary }}
                                >
                                    <Text className="text-white font-medium">
                                        {groupBuy.acceptingNewMembers ? 'Close to New Members' : 'Open to New Members'}
                                    </Text>
                                </TouchableOpacity>
